...
{
    if(c_document==nullptr)return;
    Poppler::Page *tpage = c_document->page(page);
    QSizeF sizepage = tpage->pageSize();
    QImage image = tpage->renderToImage(72*c_zoom,72*c_zoom,0,0,sizepage.width()*c_zoom,sizepage.height()*c_zoom);
